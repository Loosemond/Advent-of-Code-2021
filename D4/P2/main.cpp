...
public:
	...
	int checkWorstBoard()
	{
			...
			// for each round i will check if the board has the number
			{
				for (int l = 0; l < N; l++)
				{
					for (int c = 0; c < N; c++)
					{
						...
						{
							// cout << "board: " << boardNum << " bing: " << board[l][c] << "\n";
							lBeans[l]++;
							cBeans[c]++;
							if (lBeans[l] == 5 || cBeans[c] == 5)
							{
								// stats.at(boardNum).winRound = round;
								if (round > highestNRounds)
								{
									// cout << round << "\n";
									highestNRounds = round;
									loserBoard = boardNum;
		...
		// cout << "loser Board: " << loserBoard << " Number of rounds: " << highestNRounds << "\n";
		return finalAnser(loserBoard, highestNRounds);
	}
};
int main()
{
